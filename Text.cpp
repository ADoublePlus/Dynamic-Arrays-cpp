                ...
                const string* const other, uint length_other)
{
    ...
    // Don't need to update capacity here as this is done in resize()
}
...
string* getWords(const string* text, uint length, uint& number_unique, uint& capacity_unique)
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        {
            // Increase capacity of unique_words by 1 only if we are adding a unique word to it 
    ...
    for (int i = 0; i < length; i++)
    {
        if (!text[i].compare(word))
        {
            ...
            for (int j = i; j < length; j++)
            {
                ...
                // Overwrite the current word with the next word (shifting left)
                text[j] = text[j + 1];
            }
